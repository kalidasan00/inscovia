...
export default function RegisterInstitute() {
  ...
  const [formData, setFormData] = useState({
    instituteName: "", email: "", phone: "", password: "", confirmPassword: "",
    primaryCategory: "", secondaryCategories: [], teachingMode: "",
  ...
  // ✅ ADDED: check if user already logged in (adding new institute)
  ...
  useEffect(() => {
    ...
    // ✅ ADDED: if already logged in, pre-fill email and skip password
  ...
  const validateStep1 = () => {
    ...
    if (!instituteName || !email || !primaryCategory || !state || !district || !city || !location)
      return "Please fill in all required fields";
    if (!isExistingUser && (!phone || !password || !confirmPassword))
      return "Please fill in all required fields";
    if (!isStudyAbroad && !teachingMode) return "Please select a teaching mode";
    if (!isExistingUser && password !== confirmPassword) return "Passwords do not match";
    if (!isExistingUser && password.length < 6) return "Password must be at least 6 characters";
    return null;
  };
  ...
  const handleSendOTP = async (e) => {
    ...
    // ✅ If existing user — skip OTP, register directly
    if (isExistingUser) {
      await handleRegisterExistingUser();
      return;
    }
  ...
  // ✅ ADDED: register new org for existing logged-in user
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          password: formData.password || "existing-user",
      ...
      const newOrg = { id: data.organization.id, name: data.organization.name, city: data.organization.city, role: "OWNER" };
  ...
  const handleVerifyAndRegister = async (e) => {
    ...
    try {
      ...
      // ✅ Save both user + institute tokens
              ...
              <form onSubmit={handleSendOTP} className="space-y-7">
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className={labelClass}>Primary Category * <span className="text-gray-400 font-normal ml-1">(Main focus area)</span></label>
                    ...
                    {formData.primaryCategory && availableSecondary.length > 0 && (
                      <div>
                        <label className={labelClass}>Secondary Categories <span className="text-gray-400 font-normal ml-1">(Optional)</span></label>
                            ...
                            return (
                              <button key={cat.value} type="button"
                                ...
                                className={`px-3 py-2.5 border-2 rounded-lg text-sm font-medium transition-all text-left flex items-center justify-between gap-2 ${
                                  selected ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-700 hover:border-gray-400"
                                }`}>
                                <span>{cat.label}</span>
                                {selected && (
                    ...
                    {!isStudyAbroad && (
                      <div>
                        <label className={labelClass}>Teaching Mode * <span className="text-gray-400 font-normal ml-1">(How do you teach?)</span></label>
                          ...
                          {[
                            ...
                            <button key={mode.value} type="button"
                              ...
                              className={`py-3 px-2 border-2 rounded-lg text-sm font-medium transition-all ${
                                formData.teachingMode === mode.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-700 hover:border-gray-400"
                              }`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" name="city" value={formData.city}
                        onChange={handleChange} placeholder="Kozhikode" className={inputClass} required />
                    </div>
                    <div>
                      <label className={labelClass}>Full Address *</label>
                      <input type="text" name="location" value={formData.location}
                        onChange={handleChange} placeholder="MG Road, Near City Center" className={inputClass} required />
                    </div>
                  </div>
                </section>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="password" name="password" value={formData.password}
                          onChange={handleChange} placeholder="Min. 6 characters" className={inputClass} required />
                      </div>
                      <div>
                        <label className={labelClass}>Confirm Password *</label>
                        <input type="password" name="confirmPassword" value={formData.confirmPassword}
                          onChange={handleChange} placeholder="Repeat password" className={inputClass} required />
                      </div>
                    </div>
                  </section>
                )}
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading
                    ? (isExistingUser ? "Creating..." : "Sending OTP...")
                    : (isExistingUser ? "Add Institute" : "Continue")}
                </button>
              </form>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div className="text-center mb-8">
                ...
                <p className="text-gray-500 mt-1 text-sm">Code sent to <span className="font-semibold text-gray-800">{formData.email}</span></p>
              </div>
              {error && <div className="mb-6 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{error}</div>}
              <form onSubmit={handleVerifyAndRegister} className="space-y-5">
                ...
                <div className="text-center space-y-2">
                  ...
                  <div>
                    <button type="button" onClick={() => setStep(1)} className="text-sm text-gray-500">← Back</button>
                  </div>
                </div>
              </form>
            </>
