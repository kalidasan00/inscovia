...
export default function CenterDetails() {
  ...
  const shouldShowReadMore = (text) => text && text.length > 180;
  ...
  return (
    <>
      <Navbar />
      <main className="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pb-20 md:pb-8">
        {/* Toast */}
        {showToast && (
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-md border overflow-hidden">
          {/* Header */}
          <div className="relative h-28 sm:h-40 bg-gradient-to-br from-indigo-600 to-purple-600">
            {center.image && (
            ...
            <div className="absolute -bottom-10 sm:-bottom-12 left-4">
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white rounded-xl sm:rounded-2xl shadow-xl border-4 border-white overflow-hidden flex items-center justify-center">
                {center.logo ? (
            ...
            <div className="absolute top-3 right-3 flex gap-2">
              <button
                ...
                className={`p-2 sm:p-2.5 rounded-xl backdrop-blur-md transition-all shadow-lg ${
                  isLiked ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-white/95 text-gray-700 hover:bg-white'
                }`}
              >
              ...
              <button
                ...
                className={`p-2 sm:p-2.5 rounded-xl backdrop-blur-md transition-all shadow-lg ${
                  isComparing ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-white/95 text-gray-700 hover:bg-white disabled:opacity-50'
                }`}
              >
          ...
          <div className="pt-12 sm:pt-14 px-4 sm:px-6 pb-6">
            {/* Title & Location */}
            <div className="mb-4">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">{center.name}</h1>
            ...
            <div className="flex flex-wrap gap-2 mb-5 pb-5 border-b">
              ...
              {center.secondaryCategories?.slice(0, 2).map((cat, i) => (
                <span key={i} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-purple-100 text-purple-700">
                  {formatCategory(cat)}
                </span>
              ))}
              {center.teachingMode && (
            ...
            {categoriesWithCourses.length > 0 && (
              <div className="mb-5">
                <h2 className="text-lg font-bold text-gray-900 mb-3">Courses Offered</h2>
                ...
                {activeTab && coursesByCategory[activeTab] && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {coursesByCategory[activeTab].map((course, i) => (
                      <div key={i} className="px-3 py-2 bg-gray-50 border rounded-lg flex items-center gap-2 hover:bg-gray-100 transition-colors">
            ...
            {(center.phone || center.whatsapp || center.email || center.website) && (
              <div className="mb-5 p-4 sm:p-5 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border">
                <h2 className="text-lg font-bold text-gray-900 mb-3">Contact</h2>
                <div className="grid sm:grid-cols-2 gap-3">
                  {center.phone && (
                    <a href={`tel:${center.phone}`} className="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md transition-shadow group">
                      <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        <Phone className="w-5 h-5 text-blue-600" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs text-gray-600">Phone</p>
                        <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-blue-600">{center.phone}</p>
                      </div>
                    </a>
                  )}

                  {center.whatsapp && (
                    <a href={`https://wa.me/${center.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md transition-shadow group">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-green-600">{center.whatsapp}</p>
                      </div>
                    </a>
                  )}

                  {center.email && (
                    <a href={`mailto:${center.email}`} className="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md transition-shadow group">
                      <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        <Mail className="w-5 h-5 text-purple-600" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs text-gray-600">Email</p>
                        <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-purple-600">{center.email}</p>
                      </div>
                    </a>
                  )}

                  {center.website && (
                    <a href={center.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-white rounded-lg hover:shadow-md transition-shadow group">
                      <div className="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        <Globe className="w-5 h-5 text-indigo-600" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs text-gray-600">Website</p>
                        <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-indigo-600">Visit Website</p>
                      </div>
                    </a>
                  )}
                </div>
            ...
            {(center.facebook || center.instagram || center.linkedin) && (
              <div className="mb-5">
                <h2 className="text-lg font-bold text-gray-900 mb-3">Follow Us</h2>
                <div className="flex gap-3">
                  {center.facebook && (
                    <a href={center.facebook} target="_blank" rel="noopener noreferrer" className="w-11 h-11 bg-blue-600 rounded-xl flex items-center justify-center hover:bg-blue-700 transition-all hover:scale-105 shadow-md">
                  ...
                  {center.instagram && (
                    <a href={center.instagram} target="_blank" rel="noopener noreferrer" className="w-11 h-11 bg-gradient-to-br from-purple-600 to-pink-500 rounded-xl flex items-center justify-center hover:from-purple-700 hover:to-pink-600 transition-all hover:scale-105 shadow-md">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {center.linkedin && (
                    <a href={center.linkedin} target="_blank" rel="noopener noreferrer" className="w-11 h-11 bg-blue-700 rounded-xl flex items-center justify-center hover:bg-blue-800 transition-all hover:scale-105 shadow-md">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="mb-5">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  {center.gallery.map((img, i) => (
                    <img key={i} src={img} alt={`Gallery ${i + 1}`} className="w-full h-24 sm:h-32 object-cover rounded-lg border hover:scale-105 transition-transform" />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="pt-5 border-t mt-5">
              <Link href="/centers" className="inline-flex items-center gap-2 text-indigo-600 hover:text-indigo-700 font-semibold text-sm transition-colors">
