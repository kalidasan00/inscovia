        ...
        <div className="bg-white rounded-lg sm:rounded-xl shadow-sm overflow-hidden border">
          ...
          <div className="relative h-32 sm:h-48 bg-gradient-to-br from-accent to-accent/80">
            ...
            <div className="absolute -bottom-8 sm:-bottom-10 left-3 sm:left-4">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-lg sm:rounded-xl shadow-lg border-2 border-white overflow-hidden flex items-center justify-center">
                {center.logo ? (
          ...
          {/* Content */}
          <div className="pt-10 sm:pt-12 px-3 sm:px-4 pb-4">
            {/* Name */}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {center.secondaryCategories?.slice(0, 2).map((cat, i) => (
                <span key={i} className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-purple-100 text-purple-700">
                  {formatCategory(cat)}
                </span>
              ))}
              ...
              {center.rating > 0 && (
                <span className="inline-flex items-center gap-0.5 px-2.5 py-1 rounded-md text-xs font-medium bg-yellow-50 text-yellow-700">
                  <span>★</span>
            ...
            {(center.phone || center.whatsapp || center.email || center.website) && (
              <section className="mt-4 p-4 bg-gray-50 rounded-lg border">
                <h2 className="text-base font-semibold text-gray-900 mb-3">Contact</h2>
                <div className="space-y-2.5">
                  {center.phone && (
                    <a href={`tel:${center.phone}`} className="flex items-center gap-2.5 text-sm">
                      <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center flex-shrink-0">
                        <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg>
                      </div>
                      <span className="text-accent font-medium">{center.phone}</span>
                    </a>
                  )}

                  {center.whatsapp && (
                    <a href={`https://wa.me/${center.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-sm">
                      ...
                      <span className="text-accent font-medium">{center.whatsapp}</span>
                    </a>
                  )}

                  {center.email && (
                    <a href={`mailto:${center.email}`} className="flex items-center gap-2.5 text-sm">
                      <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center flex-shrink-0">
                        <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                      </div>
                      <span className="text-accent font-medium truncate">{center.email}</span>
                    </a>
                  )}

                  {center.website && (
                    <a href={center.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-sm">
                      <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center flex-shrink-0">
                        <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                        </svg>
                      </div>
                      <span className="text-accent font-medium truncate">Visit Website</span>
                    </a>
                  )}
                </div>
              </section>
            ...
            {(center.facebook || center.instagram || center.linkedin) && (
              <section className="mt-4">
                <h2 className="text-base font-semibold text-gray-900 mb-2">Follow Us</h2>
                <div className="flex gap-2">
                  {center.facebook && (
                    <a href={center.facebook} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-blue-600 rounded-lg flex items-center justify-center hover:bg-blue-700 transition-colors">
                  ...
                  {center.instagram && (
                    <a href={center.instagram} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-gradient-to-br from-purple-600 to-pink-500 rounded-lg flex items-center justify-center hover:from-purple-700 hover:to-pink-600 transition-colors">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {center.linkedin && (
                    <a href={center.linkedin} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-blue-700 rounded-lg flex items-center justify-center hover:bg-blue-800 transition-colors">
                      <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
            ...
            {center.gallery && center.gallery.length > 0 && (
              <section className="mt-4">
                <h2 className="text-base font-semibold text-gray-900 mb-2">Gallery</h2>
                <div className="grid grid-cols-3 gap-2">
                  {center.gallery.map((img, i) => (
                    <img key={i} src={img} alt={`Gallery ${i + 1}`} className="w-full h-24 sm:h-32 object-cover rounded-lg" />
                  ))}
                </div>
              </section>
            )}
