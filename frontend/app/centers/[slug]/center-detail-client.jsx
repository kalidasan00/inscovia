...
const InstituteMap = dynamic(() => import("../../../components/InstituteMap"), {
  ssr: false,
...
export default function CenterDetailClient({ initialCenter = null }) {
  ...
  const [loading, setLoading] = useState(!initialCenter);
  ...
  useEffect(() => {
    waLabelTimer.current = setTimeout(() => setShowWaLabel(false), 3000);
      ...
      try {
        ...
        if (!res.ok) {
          if (retryCount < MAX_RETRIES) {
            retryCount++;
            const t = setTimeout(loadCenter, 2000 * retryCount);
            retryTimers.push(t);
          } else {
            setFetchError(true);
            setLoading(false);
  ...
  const stableLogView = useCallback((...args) => logViewRef.current(...args), []);
  ...
  const truncatedDescription = useMemo(() => {
    if (!center?.description) return "";
    return center.description.length > 150
  ...
  const whatsappUrlPlain = useMemo(() => {
    if (!center?.whatsapp) return null;
    const number = center.whatsapp.replace(/\D/g, "");
          ...
          <div className="pt-12 px-3 sm:px-4 pb-4">
            ...
            <div className="flex flex-wrap gap-1.5 mb-3 pb-3 border-b">
              ...
              {center.secondaryCategories?.slice(0, 2).map((cat, i) => (
                <span key={cat || i} className="px-2 py-1 rounded-md text-xs font-medium bg-purple-100 text-purple-700">
                  {formatCategory(cat)}
                </span>
              ))}
              ...
              <div className="mb-3 pb-3 border-b">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {center.countries.map((country, i) => (
                    <span key={country || i} className="px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-700 text-xs font-medium rounded-full">
                      🌍 {country}
                    </span>
                  ))}
                </div>
              ...
              <div className="mb-3 pb-3 border-b">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {center.services.map((service, i) => (
                    <div key={service || i} className="flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 border rounded-lg">
                      <svg className="w-3.5 h-3.5 text-indigo-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              <div className="mb-3 pb-3 border-b">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {center.topUniversities.map((uni, i) => (
                    <span key={uni || i} className="px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-700 text-xs font-medium rounded-full">
                      🎓 {uni}
                    </span>
                  ))}
                </div>
            ...
            {(center.facebook || center.instagram || center.linkedin) && (
              <div className="mb-3 pb-3 border-b">
                <div className="flex items-center gap-2">
                  {center.facebook && (
                    <a href={center.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
                       className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center hover:bg-blue-700 transition-colors">
                      <FacebookIcon className="w-4 h-4 text-white" />
                    </a>
                  )}
                  {center.instagram && (
                    <a href={center.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                       className="w-8 h-8 bg-gradient-to-br from-purple-600 to-pink-500 rounded-full flex items-center justify-center">
                      <InstagramIcon className="w-4 h-4 text-white" />
                    </a>
                  )}
                  {center.linkedin && (
                    <a href={center.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
            ...
            {center.latitude && center.longitude && (
              <InstituteMap
                ...
                name={center.name}
                address={`${center.city}, ${center.state}`}
              />
            )}
