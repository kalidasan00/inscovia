...
function BottomNavInner() {
  ...
  // ✅ Save last active dashboard when pathname changes
  useEffect(() => {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      // ✅ Load lastActiveDashboard from storage
      ...
      try {
        ...
        const instData = localStorage.getItem("instituteData");
        if (instData) setInstitute(JSON.parse(instData));
  ...
  // ✅ ProfileIcon uses activeAccount which now reflects lastActiveDashboard
  const ProfileIcon = () => {
    if (!isLoggedIn) {
      return (
    ...
    return (
      <div className={`relative w-7 h-7 rounded-full ring-2 ${ringColor} ring-offset-1 ${bgColor} flex items-center justify-center`}>
        <span className="text-white text-[10px] font-bold leading-none">{label}</span>
  ...
  return (
    <>
      ...
      {/* ── Profile Bottom Sheet — long press to open ── */}
      {showSheet && (
        <>
          <div
