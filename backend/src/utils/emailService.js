...
<head>
  <style>
    ...
    .code-box { background-color: #f8f9fa; border: 2px solid #2563eb; border-radius: 6px; padding: 20px; text-align: center; margin: 25px 0; }
    .code { font-size: 32px; font-weight: bold; color: #2563eb; letter-spacing: 8px; font-family: monospace; }
    .expiry { color: #666; font-size: 13px; margin-top: 10px; }
    .button { display: inline-block; padding: 14px 35px; background-color: #2563eb; color: white; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold; }
    .note { background-color: #fff3cd; border-left: 4px solid #ffc107; padding: 15px; margin: 20px 0; font-size: 14px; color: #856404; }
    .footer { background-color: #f8f9fa; padding: 20px; text-align: center; color: #666; font-size: 12px; border-top: 1px solid #e0e0e0; }
  </style>
</head>
<body>
  <div class="container">
...
export const sendOTPEmail = async (email, otp, instituteName) => {
  try {
    ...
    if (error) { console.error('❌ Resend error:', error); throw new Error(`Failed to send email: ${error.message}`); }
...
// ✅ ADDED: Invite member email
export const sendInviteEmail = async (email, orgName, role, inviteLink) => {
  try {
    console.log(`📤 Sending invite email to: ${email} for ${orgName}`);
    const roleLabel = role === "MANAGER" ? "Manager" : "Staff";
    const content = `
      <div class="content">
        <p>Hello,</p>
        <p>You've been invited to join <strong>${orgName}</strong> on Inscovia as a <strong>${roleLabel}</strong>.</p>
        ...
        <div class="note">
          <p><strong>Important:</strong></p>
          <p>• This invite expires in 7 days<br/>
          • You'll need to create an account or login to accept<br/>
          • If you didn't expect this invite, you can ignore it</p>
        </div>
        <p>Best regards,<br/><strong>The Inscovia Team</strong></p>
      </div>
    `;
    const { data, error } = await resend.emails.send({
      from: `${FROM_NAME} <${FROM_EMAIL}>`,
      to: [email],
      subject: `You're invited to join ${orgName} on Inscovia`,
      html: getEmailTemplate(content),
    });
